...
export async function executeWithMnemo(
  ...
  const apiKey = (config.mnemoPayApiKey as string) || process.env.ANTHROPIC_API_KEY;
  ...
  if (enableFicoGating && session.ficoScore !== null && session.ficoScore < minFicoScore) {
    await ctx.onLog('stderr',
      `[MnemoPay] Agent FICO score ${session.ficoScore} is below minimum ${minFicoScore}. Execution blocked.\n`
    );
    return {
      result: { exitCode: 2 },
      updatedSession: session,
...
Respond concisely and directly. If you complete an action, confirm what was done. If you cannot complete a task, explain why.`;
  ...
  try {
    ...
    const updatedSession: MnemoPaySession = {
      ...session,
