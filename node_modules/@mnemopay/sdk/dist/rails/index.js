...
class MockRail {
    name = "mock";
    counter = 0;
    async createHold(amount, reason, agentId, _opts) {
        return {
            externalId: `mock_hold_${++this.counter}`,
            status: "held",
        };
    }
    async capturePayment(externalId, amount) {
        return {
            externalId,
            status: "captured",
...
class StripeRail {
     ...
     */
    constructor(secretKey, currency = "usd") {
     ...
     */
    static fromClient(client, currency = "usd") {
        // Bypass the require() path by stamping the field on a bare instance.
        const rail = Object.create(StripeRail.prototype);
        rail.name = "stripe";
        rail.stripe = client;
        rail.currency = currency;
        return rail;
    }
    async createHold(amount, reason, agentId, opts) {
        const params = {
    ...
    async reversePayment(externalId, _amount) {
...
class LightningRail {
     ...
     */
    static isPrivateOrReserved(hostname) {
    ...
    async lndRequest(path, method, body) {
    ...
    async capturePayment(externalId, _amount) {
        ...
        const rHashUrlSafe = externalId
            ...
            .replace(/\//g, "_")
            .replace(/=+$/, "");
        const invoice = await this.lndRequest(`/v1/invoice/${rHashUrlSafe}`, "GET");
        return {
            externalId,
...
exports.LightningRail = LightningRail;
