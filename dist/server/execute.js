...
function makeMemoryClient(config) {
    const url = config.mnemoPayServerUrl || process.env.MNEMOPAY_URL;
    if (!url)
        return null;
    const token = config.mnemoPayToken || process.env.MNEMOPAY_TOKEN;
    return new MnemoPayClient(url, token);
}
// Compute real Agent FICO score from stored execution history
function computeFico(session) {
...
export async function executeWithMnemo(ctx, session) {
    ...
    await ctx.onLog('stdout', `[MnemoPay] Agent FICO: ${fico.score} (${fico.rating}) · Trust: ${fico.trustLevel} · Runs: ${session.executionCount}\n`);
    // FICO gating — block before any API calls
    if (enableFicoGating && fico.score < minFicoScore) {
        await ctx.onLog('stderr', `[MnemoPay] Execution blocked — FICO ${fico.score} below minimum ${minFicoScore}\n`);
    ...
    try {
        ...
        if (memClient && responseText) {
            try {
                await memClient.remember(`Task "${ctx.runtime.taskKey}" completed. Result: ${responseText.slice(0, 300)}`, { importance: 0.6, tags: ['execution', ctx.agent.id] });
            }
            catch {
